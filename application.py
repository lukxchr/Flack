...
def add_user(data):
    ...
    if token and user:
        if user.token != token:
            emit('add user failed',
                 {'display_name' : display_name,
                  'message' : 'Failed to authenticate. Please choose a different display name.'})
        else:
            ...
            emit('announce users', {'users' : [user.display_name for user in users]},
                 broadcast=True)
    ...
    #new user
        ...
        if display_name in users:
            ...
            emit("add user failed",
                 {'display_name' : display_name,
                  'message' : 'Invalid display name. Please use 3-15 characters \
                  (letters, digits, _ or -)'})
        else:
    ...
    if channel_name in channels:
        emit('add channel failed',
             {'channel_name' : channel_name,
              'message': 'Channel already exists. Please choose a different name.'})
    elif not channel_name.isalnum():
        emit('add channel failed',
             {'channel_name' : channel_name,
              'message': 'Invalid channel name. Please use letters and digits only.'})
    elif len(channel_name) > 20:
        emit('add channel failed',
             {'channel_name' : channel_name,
              'message': 'Channel name too long. Please use up to 20 characters.'})
    else:
...
def join_channel(data):
    ...
    send_message({'message' : f'{user.display_name} has joined',
                  'channel' : channel_name, 'display_name' : 'admin'})
    emit('channel joined',
...
def leave_channel(data):
    ...
    send_message({'message' : f'{user.display_name} has left',
                  'channel' : channel_name, 'display_name' : 'admin'})
...
def send_priv_message(data):
    ...
    if sender and receiver:
        ...
        emit('send priv message to clients',
...
def disconnect():
    ...
    if user:
        for ch in user.joined_channels:
            send_message({'message' : f'{user.display_name} has left',
                          'channel' : ch.name, 'display_name' : 'admin'})
        users.remove(user)
        emit('announce users', {'users' : [user.display_name for user in users]},
             broadcast=True)
