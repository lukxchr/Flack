...
import {channel_template, message_template, user_template,
        priv_window_template} from './templates.js';

//init: try to load user from localStorage - if failed ask for new display name
//Add DOM/socket.io listerners
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    GLOBALS.socket.on('connect', () => {
      ...
      if (GLOBALS.display_name) {
        GLOBALS.socket.emit('add user',
          {display_name: GLOBALS.display_name, token: GLOBALS.token});
      } else {
...
function addSocketIOListeners() {
  GLOBALS.socket.on('user added', (data) => {
    ...
    if (!GLOBALS.joined_channels.includes(GLOBALS.current_channel)) {
      ...
      GLOBALS.socket.emit('load channel',
        {channel_name: GLOBALS.current_channel});
  });
  ...
  GLOBALS.socket.on('channel joined', (data) => {
    ...
    document.querySelector(`
      li.channel[data-channel=${channel_name}]`).classList.add("joined-channel");
  ...
  GLOBALS.socket.on('send message to clients', (data) => {
    ...
    message_area.innerHTML += message_template(
        {'sender' : msg['sender'], 'timestamp' : msg['timestamp'].slice(11,16),
        'content' : msg['content'],
  ...
  GLOBALS.socket.on('channel left', (data) => {
    ...
    const channel_name = data['channel_name'];
    document.querySelector(`
      li.channel[data-channel=${channel_name}]`)
      .classList.remove("joined-channel", 'current-channel');
    document.querySelector('#messages-window').innerHTML =
      '<h1>Choose channel to send and receive messages.</h1>';
  });
  GLOBALS.socket.on('channel loaded', (data) => {
    ...
    document.querySelector(`li.channel[data-channel=${channel_name}]`)
    .classList.add("current-channel");
  });
  ...
  GLOBALS.socket.on('send priv message to clients', (data) => {
    ...
    const message = message_template(
        {sender: sender,
         timestamp: data['message']['timestamp'].slice(11,16),
         content: data['message']['content'],
         priv_btn: false});

    let priv_window;
    if (GLOBALS.display_name === sender) {
      priv_window = document.querySelector(`
        .priv-msg-container[data-receiver=${receiver}]`);
    } else if (GLOBALS.display_name === receiver) {
      priv_window = document.querySelector(`
        .priv-msg-container[data-receiver=${sender}]`);
      if (!priv_window) {
  ...
  document.addEventListener('click', (e) => {
    if (e.target.matches('#send-button')) {
      ...
      GLOBALS.socket.emit('join channel',
        {channel_name: channel_name, display_name: GLOBALS.display_name,
          token: GLOBALS.token});
      document.querySelector(`
        li.channel[data-channel=${channel_name}]`).classList.add("joined-channel");
      ...
      GLOBALS.socket.emit('leave channel',
        {channel_name: channel_name, display_name: GLOBALS.display_name,
         token: GLOBALS.token});
      document.querySelector(`li.channel[data-channel=${channel_name}]`)
      .classList.remove("joined-channel");
...
function sendMessage() {
  ...
  GLOBALS.socket.emit('send message to server',
    {message: message, channel: GLOBALS.current_channel,
     display_name: GLOBALS.display_name});
  message_input.value = '';
}

function sendPrivateMessage(receiver) {
  const priv_window = document.querySelector(`
    .priv-msg-container[data-receiver=${receiver}]`);
  const message_input = priv_window.querySelector('.priv-msg-input');
  GLOBALS.socket.emit('send priv message to server',
    {message: message_input.value, sender: GLOBALS.display_name,
     receiver: receiver, token: GLOBALS.token});
  message_input.value = '';
}
...
function renderChannelList() {
  ...
  GLOBALS.channels.forEach(ch => {
    ...
    if (GLOBALS.current_channel === ch)
      document.querySelector(`li.channel[data-channel=${ch}]`)
      .classList.add("current-channel");
    if (GLOBALS.joined_channels.includes(ch))
      document.querySelector(`li.channel[data-channel=${ch}]`)
      .classList.add("joined-channel");
  });
}
...
function renderOnlineUsersList(users) {
  ...
  users.forEach(display_name => {
    const user = user_template({display_name: display_name,
                 priv_btn: display_name != GLOBALS.display_name});
